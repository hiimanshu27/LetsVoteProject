...
<?php
    ...
    if($candidate == "lib"){
        $query = "INSERT INTO Candidates (Justin, Andrew, Jagmeet, Elizabeth, username) VALUES ('1','0','0','0','$user')";  
        mysqli_query( $link, $query );
    }
    else if($candidate == "con"){
        $query = "INSERT INTO Candidates (Justin, Andrew, Jagmeet, Elizabeth, username) VALUES ('0','1','0','0','$user')";    
        mysqli_query( $link, $query );
    }
    else if($candidate == "ndp"){
        $query = "INSERT INTO Candidates (Justin, Andrew, Jagmeet, Elizabeth,username) VALUES ('0','0','1','0','$user')";    
        mysqli_query( $link, $query );
    }
    else if($candidate == "gp"){
        $query = "INSERT INTO Candidates (Justin, Andrew, Jagmeet, Elizabeth,username) VALUES ('0','0','0','1','$user')";    
        mysqli_query( $link, $query );
    }
    ...
    $jsql = "SELECT Justin FROM Candidates WHERE Justin=1 ";
    $jresult =  mysqli_query($link,$jsql);
    $justinVotes = mysqli_num_rows($jresult);
    $jans = sprintf('%0.2f',($justinVotes/$rowcount)*100);
    
    $asql = "SELECT Andrew FROM Candidates WHERE Andrew=1 ";
    $aresult =  mysqli_query($link,$asql);
    $AndrewVotes = mysqli_num_rows($aresult);
    $aans = sprintf('%0.2f',($AndrewVotes/$rowcount)*100);
    
    $ssql = "SELECT Jagmeet FROM Candidates WHERE Jagmeet=1 ";
    $sresult =  mysqli_query($link,$ssql);
    $jagmeetVotes = mysqli_num_rows($sresult);
    $sans = sprintf('%0.2f',($jagmeetVotes/$rowcount)*100);
    
    $esql = "SELECT Elizabeth FROM Candidates WHERE Elizabeth=1 ";
    $eresult =  mysqli_query($link,$esql);
    $elizabethVotes = mysqli_num_rows($eresult);
    $eans = sprintf('%0.2f', ($elizabethVotes/$rowcount)*100);
...
<head>
    ...
    <title>LETS VOTE | THANKS FOR VOTING</title>
  ...
  <header>
    ...
    <div class="modal fade" id="myModal" role="dialog">
        ...
        <div class="modal-body">
            ...
            <ul class="state-list" id="fifthNav">
    ...
    <div class="poll">
    <h2>General Election Survey 2019</h2>
    <small class="text-muted" style="color:black">Thanks for your vote</small>
    </div>
    <ol>
